...
func ValidateAndBuildNodeBIdKey(nodeType string, plmnId string, nbId string, cuupId string, duId string) (string, error) {
	...
	if cuupId != "0" && cuupId != "" && duId != "0" && duId != ""{
		return fmt.Sprintf("%s:%s:%s:%s:%s", nodeType, plmnId, cuupId, duId, nbId), nil
	}else if cuupId != "0" && cuupId != "" {
		return fmt.Sprintf("%s:%s:%s:%s", nodeType, plmnId,cuupId, nbId), nil
	}else if duId != "0" && duId != "" {
		return fmt.Sprintf("%s:%s:%s:%s", nodeType, plmnId,duId, nbId), nil
	}else {
